...
export function AgentsModal() {
  ...
  const [form, setForm] = useState({ name: '', role: '', tone: 'debate' as AgentTone, color: AGENT_COLORS[0], prompt: '' })

  const reset = () => { setEditId(null); setForm({ name: '', role: '', tone: 'debate', color: AGENT_COLORS[0], prompt: '' }) }
  const handleEdit = (a: typeof agents[0]) => { setEditId(a.id); setForm({ name: a.name, role: a.roleTitle, tone: a.tone, color: a.avatarColor, prompt: a.systemPrompt }) }
  const handleSave = () => {
    ...
    <Modal open onClose={() => { reset(); closeModal() }} title="Manage Agents" description="Create and configure your AI agents." maxWidth="max-w-xl">
      <div className="space-y-3 max-h-40 overflow-y-auto">
        {agents.map((a) => (
          <div key={a.id} className={cn('flex items-center gap-2.5 px-3 py-2 rounded-lg', editId === a.id ? 'bg-surface-hover' : 'hover:bg-surface-hover')}>
            ...
            <div className="flex-1 min-w-0"><div className="text-xs font-medium text-ink">{a.name}</div><div className="text-[11px] text-ink-faint">{a.roleTitle}</div></div>
            <button onClick={() => handleEdit(a)} className="text-[11px] text-ink-faint hover:text-ink px-2 py-1 rounded transition-colors">Edit</button>
            <button onClick={() => removeAgent(a.id)} className="text-[11px] text-ink-faint hover:text-red-600 px-2 py-1 rounded transition-colors">Del</button>
          </div>
        ))}
      </div>
      <div className="border-t border-border pt-4 mt-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-[11px] text-ink-faint block mb-1">Name</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-3 py-2 text-sm bg-surface-inset border border-border rounded-md focus:outline-none focus:border-ink-faint" /></div>
          <div><label className="text-[11px] text-ink-faint block mb-1">Role</label><input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="w-full px-3 py-2 text-sm bg-surface-inset border border-border rounded-md focus:outline-none focus:border-ink-faint" /></div>
        </div>
        <div><label className="text-[11px] text-ink-faint block mb-1">Tone</label>
        ...
        <div><label className="text-[11px] text-ink-faint block mb-1">Color</label>
          <div className="flex gap-2">{AGENT_COLORS.map((c) => (<button key={c} onClick={() => setForm({ ...form, color: c })} className={cn('w-6 h-6 rounded-full transition-transform', form.color === c && 'ring-2 ring-offset-2 ring-sand-700')} style={{ backgroundColor: c }} />))}</div>
        </div>
        <div><label className="text-[11px] text-ink-faint block mb-1">System Prompt</label>
